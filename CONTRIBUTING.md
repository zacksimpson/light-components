...
1. Pick a category: `components/stock/` if it recreates an existing LightOS screen, `components/original/` if it's your own design (see the README's [About Components](README.md#about-components) for what each means). Match the shape below, copying an existing component's folder as a starting point (`components/stock/date-picker/` is a solid one):

   | File | What it needs |
   | :--- | :--- |
   | `build.gradle.kts` | Namespace under `com.thelightphone.components.<name>` |
   | `README.md` | What it does, `Depends on`, `Pasting this in` |
   | `src/main/kotlin/com/thelightphone/components/<name>/` | The screen itself |
